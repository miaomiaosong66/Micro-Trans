#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time          : 2025/01/17 17:16
# @Author        : jcfszxc
# @Email         : jcfszxc.ai@gmail.com
# @File          : 3_DSM_HFEM_FIM_USM_FTM.py
# @Description   : 
...
def apply_feature_transformation(val_probas_raw, model, X_val, y_val, 
                               temperature=1.0, smoothing_factor=0.1):
    """
    Apply the complete feature transformation pipeline.
    ...
    Returns:
        np.ndarray: Final transformed probabilities
    """
    # 1. Apply feature-based transformation
    transformed_probas = transform_prediction_probabilities(
        val_probas_raw, 
        model, 
        X_val,
...
def cross_validate_model(X, y, class_labels=None, n_iterations=15):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # 4. 应用特征变换模块
        print("\nApplying Feature Transform Module...")
        transformed_probas = apply_feature_transformation(
            val_probas_raw,
            best_model,
            X_val,
            y_val,
...
def save_results(results):
    ...
    output_file = output_dir / f'MASFI/DSM+HFEM+FIM+USM+FTM_results_{timestamp}.json'
