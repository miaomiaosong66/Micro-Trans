#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time          : 2025/01/18 16:46
# @Author        : jcfszxc
# @Email         : jcfszxc.ai@gmail.com
# @File          : 4_DSM_HFEM_MFFM_USM_FTM.py
# @Description   : 
...
def train_evaluate_mlp(X, y):
    """Initialize and return Multi-Layer Perceptron model with specified parameters"""
...
def get_mlp_feature_importance(model, X, y):
    """
    ...
    baseline_score = roc_auc_score(y, model.predict_proba(X), multi_class='ovr')
    ...
    for i in range(X.shape[1]):
        ...
        permuted_score = roc_auc_score(y, model.predict_proba(X_permuted), multi_class='ovr')
        importance_scores[i] = baseline_score - permuted_score
    
    # Normalize importance scores
    importance_scores = (importance_scores - np.min(importance_scores)) / \
                       (np.max(importance_scores) - np.min(importance_scores) + 1e-10)
...
def apply_feature_transformation_with_attention(val_probas_raw, model, X_val, X_val_transformed, y_val, 
    ...
    """
    Enhanced feature transformation with feature fusion, modified for MLP support.
    
    Args:
        val_probas_raw: Raw prediction probabilities from MLP
        model: Trained MLP model
    ...
    feature_importance = get_mlp_feature_importance(model, X_val, y_val)
    ...
    combined_features = np.concatenate([
        ...
        X_val_transformed,  # Transformer processed features
        val_probas_raw  # MLP's prediction probabilities
    ], axis=1)
    
    # Apply temperature scaling and probability smoothing
    transformed_probas = np.zeros_like(val_probas_raw)
    
    for i in range(len(X_val)):
        # Get activation patterns from hidden layers
        # Note: This is MLP-specific confidence estimation
        layer_activations = []
        current_layer = X_val[i:i+1]
        
        # Forward pass through each layer
        for layer_idx in range(len(model.coefs_)):
            current_layer = np.dot(current_layer, model.coefs_[layer_idx]) + model.intercepts_[layer_idx]
            if layer_idx < len(model.coefs_) - 1:  # Apply ReLU to hidden layers
                current_layer = np.maximum(0, current_layer)
            layer_activations.append(current_layer)
        
        # Calculate confidence based on activation patterns
        activation_confidence = np.mean([np.std(act) for act in layer_activations])
        activation_confidence = 1 / (1 + np.exp(-activation_confidence))  # Sigmoid scaling
        ...
        # Combine with activation-based confidence
        feature_weight = 0.7  # Weight for feature-based confidence
        activation_weight = 0.3  # Weight for activation-based confidence
        
        confidence_weight = (feature_weight * np.mean(feature_importance) + 
                           activation_weight * activation_confidence)
    ...
    return adaptive_confidence_calibration(transformed_probas, y_val)
...
def cross_validate_model(X, y, class_labels=None, n_iterations=15):
    """Modified cross-validation training process for MLP"""
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    fold_metrics = []
    
    n_classes = len(np.unique(y))
    
    print("\nTraining MLP with enhanced confidence calibration...")
    print("-" * 50)
    
    # Initialize Transformer feature processor
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        for iteration in range(n_iterations):
            ...
            # X_train_sample = np.concatenate([X_train_sample, X_train_scaled])
            # y_train_sample = np.concatenate([y_train_sample, y_train])
            
            # Train model
            model = train_evaluate_mlp(X_train_sample, y_train_sample)
            ...
            # Update best model (considering AUC-ROC)
            if val_auc > best_auc or (val_auc == best_auc and val_score > best_score):
                best_score = val_score
                best_auc = val_auc
                best_model = model
            
            # Get prediction probabilities and update sampling weights
        ...
        X_val_original = X_val_scaled.copy()
        
        # 2. Transformer processed features
        print("- Applying Transformer attention")
        X_val_transformed = feature_processor.process_features(X_val).numpy()
        X_val_transformed = scaler.fit_transform(X_val_transformed)  # Scale transformer features
        
        # 3. Get MLP predictions
        print("- Getting MLP predictions")
        val_probas_raw = best_model.predict_proba(X_val_scaled)
        
        # Apply enhanced feature transform module
        ...
        metrics_before = calculate_metrics(y_val, best_model.predict(X_val_scaled), 
...
def save_results(results):
    ...
    output_file = output_dir / f'MASFI/DSM+HFEM+MFFM+USM+FTM_results_{timestamp}.json'
